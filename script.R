...
ggplot(data = count_traffic_accident_by_year, aes(x = 사고년도, y = count)) + geom_line() + geom_point()
...
tmp14 <- traffic_accident %>% group_by(가해자법규위반) %>% summarise(n = n())
ggplot(data=tmp14, aes(x=가해자법규위반, y=n)) + geom_bar(stat = "identity", position=position_dodge(), fill='#FFA800') + geom_text(aes(label=n), color='black')
...
tmp8 <- traffic_accident %>% group_by(`도로형태 대분류`) %>% summarise(총사망자=sum(사망자수), 총부상자=sum(부상자수), 평균=sum(사망자수)/sum(부상자수))
...
tmp9 <- traffic_accident %>% group_by(`사고유형 중분류`) %>% summarise(사망자수 = sum(사망자수), 부상자수 = sum(부상자수), 중상자수 = sum(중상자수), 경상자수 = sum(경상자수))
